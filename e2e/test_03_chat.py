...
class TestChat:
    def test_chat_input_field_visible(self, at_home):
        screens = at_home
        screens.home.tap_chat_tab()
        assert screens.chat.is_visible()

    def test_send_button_disabled_on_empty_input(self, at_home):
        screens = at_home
        screens.home.tap_chat_tab()
    ...
    def test_send_message_gets_response(self, at_home):
        screens = at_home
        screens.home.tap_chat_tab()
        response = _ask_and_get(screens, "What is my moon sign?")
        assert len(response) > 50

    def test_response_is_not_empty(self, at_home):
        screens = at_home
        screens.home.tap_chat_tab()
        response = _ask_and_get(screens, "Give me a brief daily outlook")
        assert response.strip() != ""

    def test_loading_indicator_shown_during_response(self, driver, at_home):
        screens = at_home
        from appium.webdriver.common.appiumby import AppiumBy
        screens.home.tap_chat_tab()
    ...
    def test_copy_button_appears_on_response(self, at_home):
        screens = at_home
        screens.home.tap_chat_tab()
        _ask_and_get(screens, "Brief career prediction")
        assert screens.chat.present("copy_message_button")

    def test_new_chat_clears_messages(self, at_home):
        screens = at_home
        screens.home.tap_chat_tab()
    ...
    def test_history_button_opens_history(self, at_home):
        screens = at_home
        screens.home.tap_history()
        assert screens.history.is_visible()

    def test_history_shows_previous_threads(self, at_home):
        screens = at_home
        screens.home.tap_history()
        count = screens.history.thread_count()
        assert count >= 0  # may be empty on fresh install

    def test_open_thread_loads_messages(self, at_home):
        screens = at_home
        screens.home.tap_chat_tab()
    ...
    def test_chart_button_appears_for_planet_response(self, at_home):
        screens = at_home
        screens.home.tap_chat_tab()
    ...
    def test_send_finance_question_gets_response(self, at_home):
        screens = at_home
        screens.home.tap_chat_tab()
